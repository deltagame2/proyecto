...
async function llamarLibros(genero) {
  try {
    ...
    console.log("localhost/aproyecto/php/ventaLibros.php?titulo=" + titulo + "&autor=" + autor + "&editorial=" + editorial + "&genero=" + genero);
    let response = await fetch("https://booksell.store/php/ventaLibros.php?titulo=" + titulo + "&autor=" + autor + "&editorial=" + editorial + "&genero=" + genero, {
      method: "GET",
  ...
  `;
    ...
    for (let i = 0; i < librosPorPagina; i++) {
      ...
      let email = enTexto[i].emailVendedor;
        ...
        <div class="text-center">   <img width="80%" class="blur" height="250" src="`+ imagenLibro + `"></div>
          ...
          <div class="card-body">
            ...
            <div class="d-flex justify-content-between align-items-center">
              <div class="btn-group">
                <button type="button" class="btn btn-sm btn-outline-secondary verMas" >${precio} €</button>
    ...
    window.onscroll = function () {
      ...
      for (let i = 0; i < document.getElementsByClassName("corazon").length; i++) {
        ...
        corazon.addEventListener("click", async function () {
          ...
          let id_venta = document.getElementById(i).value;
  
          await Favoritos(id_venta, emailf);
...
async function Favoritos(id_venta, email) {
  ...
  try {
    if (window.sessionStorage.getItem("email") == null) {
      ...
      let response = await fetch("https://booksell.store/php/favoritosV.php?idVenta=" + id_venta + "&email=" + email, {
        method: "GET",
        headers: { "Content-type": "application/json" }
      });
...
async function generos() {
  try {
    let response = await fetch("https://booksell.store/php/muestraGeneroV.php", {
      method: "GET",
      headers: { "Content-type": "application/json" }
    });
